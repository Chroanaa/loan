...
$sql = "UPDATE loan_applications SET status = 'Rejected' WHERE loan_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $loan_id);

if ($stmt->execute()) {
    // Insert notification for rejected loan
    $message = "Your loan application with ID $loan_id has been rejected.";
    ...
    $_SESSION['success'] = "Loan application rejected successfully.";
} else {
    $_SESSION['errors'] = ["Failed to reject loan application."];
}
