...
$sql = "SELECT l.loan_id, u.name AS client_name, l.loan_amount, l.loan_term, l.interest_rate, l.status, l.created_at, l.updated_at, b.branch_name
        ...
        WHERE u.branch_id = ? AND YEARWEEK(l.created_at, 1) = YEARWEEK(CURDATE(), 1)";
...
$sql = "SELECT l.loan_id, u.name AS client_name, l.loan_amount, l.loan_term, l.interest_rate, l.status, l.created_at, l.updated_at, b.branch_name
        ...
        JOIN user_tbl u ON l.client_id = u.user_id
        JOIN branches b ON u.branch_id = b.branch_id
        WHERE u.branch_id = ? AND l.status = 'Approved' AND DATE_ADD(l.updated_at, INTERVAL l.loan_term MONTH) < NOW() AND YEARWEEK(l.updated_at, 1) = YEARWEEK(CURDATE(), 1)";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $branch_id);
$stmt->execute();
$result = $stmt->get_result();
