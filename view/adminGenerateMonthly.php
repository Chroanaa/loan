...
$sql = "SELECT l.loan_id, u.name AS client_name, l.loan_amount, l.loan_term, l.interest_rate, l.status, l.created_at, l.updated_at, b.branch_name
        ...
        WHERE MONTH(l.created_at) = MONTH(CURDATE()) AND YEAR(l.created_at) = YEAR(CURDATE())";
...
$sql = "SELECT l.loan_id, u.name AS client_name, l.loan_amount, l.loan_term, l.interest_rate, l.status, l.created_at, l.updated_at, b.branch_name
        ...
        JOIN user_tbl u ON l.client_id = u.user_id
        JOIN branches b ON u.branch_id = b.branch_id
        WHERE l.status = 'Approved' AND DATE_ADD(l.updated_at, INTERVAL l.loan_term MONTH) < NOW() AND MONTH(l.updated_at) = MONTH(CURDATE()) AND YEAR(l.updated_at) = YEAR(CURDATE())";
$stmt = $conn->prepare($sql);
$stmt->execute();
$result = $stmt->get_result();
