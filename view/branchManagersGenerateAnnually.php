...
$sql = "SELECT l.loan_id, u.name AS client_name, l.loan_amount, l.loan_term, l.interest_rate, l.status, l.created_at, l.updated_at, b.branch_name
        ...
        WHERE u.branch_id = ? AND YEAR(l.created_at) = YEAR(CURDATE())";
...
// Fetch overdue loans for the current year
$sql = "SELECT l.loan_id, u.name AS client_name, l.loan_amount, l.loan_term, l.interest_rate, l.status, l.created_at, l.updated_at, b.branch_name
        FROM loan_applications l
        JOIN user_tbl u ON l.client_id = u.user_id
        JOIN branches b ON u.branch_id = b.branch_id
        WHERE u.branch_id = ? AND l.status = 'Approved' AND DATE_ADD(l.updated_at, INTERVAL l.loan_term MONTH) < NOW() AND YEAR(l.updated_at) = YEAR(CURDATE())";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $branch_id);
$stmt->execute();
$result = $stmt->get_result();
...
header('Content-Disposition: attachment;filename=annual_report.csv');
